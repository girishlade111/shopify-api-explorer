...
interface AppProps {
  ...
  tools: any[];
  isVoiceMode?: boolean;
  onSwitchToVoiceMode?: () => void; // Add this prop to receive the function
}
...
function CopilotDemoApp(props: AppProps) {
  ...
  return (
    <div className={`flex flex-col bg-white ${props.isVoiceMode ? 'h-auto' : 'h-[440px]'}`}>
      <div className={`flex flex-1 overflow-hidden relative ${props.isVoiceMode ? 'min-h-0' : ''}`}>
        <Transcript
          ...
          onSwitchToVoiceMode={props.onSwitchToVoiceMode} // Pass the function down
        />
      </div>
    </div>
  );
