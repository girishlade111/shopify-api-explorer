...
export default function FittingRoomPage() {
  ...
  const renderProductGrid = (productSet: FittingRoomProduct[]) => <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {productSet.map(product => <Card key={product.variant_id} className="overflow-hidden">
          <div className="aspect-[3/4] relative overflow-hidden bg-gray-100">
            <img src={product.image_url} alt={product.title} className="object-cover w-full h-full transition-transform duration-300 hover:scale-105" />
          </div>
          ...
          <CardFooter className="p-4 pt-0 flex gap-2">
            ...
            <Button className="flex-1 text-white" onClick={() => handleAddToCart(product.product_id, product.variant_id)}>
  ...
  const renderCompareTable = (productSet: FittingRoomProduct[]) => <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody>
          {productSet.map(product => <tr key={product.variant_id} className="border-b">
              <td className="p-4">
              ...
              <td className="p-4">
                <div className="flex gap-2">
                  ...
                  <Button size="sm" onClick={() => handleAddToCart(product.product_id, product.variant_id)}>
                    Add to Cart
                  </Button>
                </div>
              </td>
  ...
  return <div className="container py-12">
      ...
      {products.length === 0 ? <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          {/* Product Set Tabs */}
          {tabs.length > 0 && <Tabs value={activeTabId} onValueChange={setActiveTabId} className="w-full">
              <TabsList className="mb-6 flex flex-wrap">
                {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="flex-grow">
